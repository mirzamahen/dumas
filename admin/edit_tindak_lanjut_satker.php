...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $tanggapan_balik = $_POST['tanggapan_satker'];

    // Prepare an update statement
    $sql = "UPDATE pengaduan 
            SET tanggapan_satker=?
            WHERE id_pengaduan=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("si", $tanggapan_balik, $id);
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Berikan Klarifikasi Satker Anda</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <form method="POST" action="edit_tindak_lanjut_satker.php">
                  <input type="hidden" name="id_pengaduan" value="<?php echo $row['id_pengaduan']; ?>">
                  <div class="form-group">
                    <label for="tanggapan_satker">Tindak Lanjut:</label>
                    <textarea class="form-control" id="tanggapan_satker" name="tanggapan_satker" rows="3"><?php echo $row['tanggapan_satker']; ?></textarea>
                  </div>
                  <button type="submit" class="btn btn-primary">Update</button>
                  <a href="tindak_lanjut_satker.php" class="btn btn-default">Cancel</a>
                </form>
              </div>
              <!-- /.card-body -->
            </div>
