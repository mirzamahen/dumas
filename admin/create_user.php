...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO users_admin (username, password, nama, nip, satker_id) VALUES ('$username', '$hashed_password', '$nama', '$nip', '$satker_id')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('User Berhasil Ditambah'); window.location.href='manage_user.php';</script>";
    } else {
...
<head>
  ...
  <title>Add User | Matur Bapak Yooo</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-2">
            <button type="button" class="btn btn-block btn-warning" onclick="window.location.href='manage_user.php'">Kembali</button>
          </div>
        </div>
      </div>
    </section>
                    ...
                    <select class="form-control" id="satker_id" name="satker_id" required>
                      ...
                      if ($satker_result->num_rows > 0) {
                          while ($satker = $satker_result->fetch_assoc()) {
                              echo "<option value='".$satker["satker_id"]."'>".$satker["nama_satker"]."</option>";
                          }
                      }
                      ?>
                    </select>
